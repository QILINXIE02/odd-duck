...
// Function to retrieve products data from local storage
function getProductsData() {
  const storedData = localStorage.getItem('productsData');
  if (storedData) {
      const parsedData = JSON.parse(storedData);

      // If your constructor utilizes prototype methods, recreate the objects
      const productsArray = parsedData.map(data => new Product(data.name, data.imagePath));

      return productsArray;
  }

  return null; // Return null if there's no data in local storage
}

// Function to initialize products from local storage or default data
function initializeProducts() {
  ...
  if (storedProducts) {
      ...
      // Initialize products array with default data
